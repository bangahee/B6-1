...
> 실습 결과: `2026-09-04` 배포·내부/외부 검증·리소스 정리 완료<br>
> 현재 상태: 사용자 정의 AWS 리소스 삭제 완료 / 정리 직후 예상 청구액 `USD 0.00`
...
- 서울 리전의 사용자 정의 VPC와 Public Subnet에 Ubuntu 24.04 LTS `t3.micro` EC2를 생성했다.
- Nginx로 정적 사이트를 배포하고 EC2 내부 및 외부에서 `GET /health → HTTP 200 / OK`를 확인했다.
- Security Group은 HTTP 80을 공개하고 SSH 22를 학습자 공인 IP `/32`로 제한했다.
- 증빙 저장 후 EC2, EBS와 사용자 정의 VPC 관련 리소스를 모두 삭제했다.
- 실제 결과와 증빙은 [`../README.md`](../README.md), [`troubleshooting.md`](troubleshooting.md), [`cleanup-checklist.md`](cleanup-checklist.md)에서 확인할 수 있다.

아래 절차와 체크박스는 같은 구성을 다시 실습할 때 사용하는 재실행용 가이드다. 개인 IAM 사용자 이름, 로컬 디렉터리와 키 경로는 저장소에 기록하지 않고 각자의 환경에 맞게 바꾼다.
...
위 상세 절차를 한 번 이해한 뒤 같은 실습을 다시 수행할 때 사용하는 템플릿이다. 아래의 빈 체크박스는 현재 프로젝트가 미완료라는 뜻이 아니다.
