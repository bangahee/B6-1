...
VPC를 처음 접할 때는 **VPC를 하나의 개인 도시**, **Subnet을 도시 안에서 나눈 구역**, **IP 주소를 각 건물의 주소**라고 생각할 수 있다.

```text
VPC라는 전체 도시: 10.0.0.0/16
│
├── 이번 Public Subnet: 10.0.1.0/24
│   └── 이 범위 안에서 EC2가 Private IPv4를 받음
│
├── 추가 가능한 Subnet 예시: 10.0.2.0/24
└── 추가 가능한 Subnet 예시: 10.0.3.0/24
```

IPv4 주소는 `10.0.1.0`처럼 점으로 구분된 네 칸으로 표시하며, 각 칸은 8비트로 이루어져 있다. 따라서 전체 IPv4 주소는 `8 × 4 = 32비트`다. 뒤의 `/숫자`는 앞에서부터 네트워크 주소로 고정할 비트 수를 의미한다.

#### VPC의 `10.0.0.0/16`

`/16`은 앞의 16비트, 즉 앞의 두 칸 `10.0`을 네트워크 영역으로 고정한다. 나머지 두 칸이 변할 수 있으므로 범위는 다음과 같다.

```text
10.0.0.0 ~ 10.0.255.255
```

변할 수 있는 비트는 `32 - 16 = 16비트`이므로 전체 주소 개수는 `2¹⁶ = 65,536개`다. 이 범위가 프로젝트 VPC 안에서 Subnet을 나누고 리소스에 Private IPv4를 배정할 수 있는 전체 공간이다.

#### Public Subnet의 `10.0.1.0/24`

`/24`는 앞의 24비트, 즉 앞의 세 칸 `10.0.1`을 고정한다. 마지막 한 칸만 `0`부터 `255`까지 변할 수 있다.
...
마지막 칸은 8비트이므로 만들 수 있는 조합은 `2⁸ = 256개`다. 비트 하나는 `0` 또는 `1`이라는 두 가지 값만 가질 수 있다. 8개의 비트가 각각 두 가지 값을 선택할 수 있으므로 가능한 조합은 다음과 같이 계산한다.
...
비트 수가 늘어날 때마다 가능한 조합의 수는 두 배가 된다.

| 비트 수 | 조합 예시 | 조합 개수 |
|---:|---|---:|
| 1비트 | `0`, `1` | `2¹ = 2개` |
| 2비트 | `00`, `01`, `10`, `11` | `2² = 4개` |
| 3비트 | `000`부터 `111` | `2³ = 8개` |
| 8비트 | `00000000`부터 `11111111` | `2⁸ = 256개` |

8비트의 가장 작은 값 `00000000`은 십진수 `0`이고, 가장 큰 값 `11111111`은 십진수 `255`다. 따라서 마지막 칸은 `0`부터 `255`까지 변한다. `0`도 하나의 값으로 포함되기 때문에 `0~255`는 255개가 아니라 256개다.
...
같은 계산을 `2^(32 - 24) = 256`으로 표현할 수 있다.

AWS는 각 Subnet에서 처음 네 주소와 마지막 한 주소를 예약한다.

| 예약 주소 | 용도 |
|---|---|
| `10.0.1.0` | Subnet 자체를 나타내는 Network address |
| `10.0.1.1` | VPC Router |
| `10.0.1.2` | AWS가 제공하는 DNS |
| `10.0.1.3` | AWS가 향후 사용하기 위해 예약 |
| `10.0.1.255` | Network broadcast 주소로 예약 |

따라서 `10.0.1.0/24`에는 전체 256개 주소가 있지만 EC2 같은 리소스에 실제로 할당할 수 있는 주소는 `256 - 5 = 251개`다.

`/16`보다 `/24`의 숫자가 더 크지만 주소 범위는 더 작다. `/` 뒤의 숫자가 클수록 고정되는 비트가 많아지고, 리소스 주소로 사용할 수 있는 나머지 공간은 줄어들기 때문이다.

또한 **Public Subnet이라는 이름이 `10.0.1.x` Private IPv4를 인터넷에 직접 공개한다는 뜻은 아니다.** Subnet이 Public이 되려면 Internet Gateway로 향하는 Route가 있어야 하며, EC2가 외부와 직접 통신하려면 Public IPv4와 적절한 Security Group 규칙도 필요하다.
